...
import { PromptBuilder } from "./dist/core/PromptBuilder.js";
...
const config = {
  ...
  router: "vue-router",
  state: "vuex",
  tools: ["vue2", "less", "element-ui", "npm"],
};

const prompt = await PromptBuilder.build(config);

// 提取依赖部分
const startIdx = prompt.indexOf("## 📦 五、依赖包清单");
const endIdx = prompt.indexOf("## 💡 六、开发规范");

if (startIdx !== -1 && endIdx !== -1) {
  console.log(prompt.substring(startIdx, endIdx));
} else {
  console.log("未找到依赖部分");
...
async function finalVerificationTest() {
  ...
  try {
    ...
    console.log(
      `   ✅ React项目生成: ${reactResult.success ? "成功" : "失败"}`
    );
    ...
    console.log(
      `   ✅ 向后兼容测试: ${backwardResult.success ? "成功" : "失败"}`
    );
    ...
    if (fs.existsSync(vue3ProjectPath)) {
      ...
      if (fs.existsSync(packageJsonPath)) {
        ...
        console.log(
          `   🔧 依赖数量: ${Object.keys(packageJson.dependencies || {}).length}`
        );
        console.log(
          `   🛠️  开发依赖数量: ${Object.keys(packageJson.devDependencies || {}).length}`
        );

        // 检查关键依赖
        const hasPinia =
          packageJson.dependencies?.pinia || packageJson.devDependencies?.pinia;
        const hasRouter =
          packageJson.dependencies?.["vue-router"] ||
          packageJson.devDependencies?.["vue-router"];
        const hasElementPlus =
    ...
    console.log(
      `   - Vue3完整项目: ${vue3Result.success ? "✅ 通过" : "❌ 失败"}`
    );
    console.log(
      `   - React项目: ${reactResult.success ? "✅ 通过" : "❌ 失败"}`
    );
    console.log(
      `   - 向后兼容性: ${backwardResult.success ? "✅ 通过" : "❌ 失败"}`
    );

    const allPassed =
      vue3Result.success && reactResult.success && backwardResult.success;
    console.log(
