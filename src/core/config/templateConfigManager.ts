...
export class TemplateConfigManager {
  ...
  constructor(options?: { cacheDir?: string; reloadInterval?: number }) {
    ...
    this.cacheDir = options?.cacheDir || path.join(projectRoot, '.template-cache', 'config');
    ...
    logger.info(`[TemplateConfigManager] 重载间隔: ${this.reloadInterval / 1000}秒`);
  }
  ...
  private getRemoteConfigUrl(localConfig: any): string | null {
    try {
      if (localConfig?.remoteConfig?.enabled && localConfig?.remoteConfig?.repository?.url) {
        ...
        this.addLog(`[TemplateConfigManager] 从本地配置解析远程URL: ${remoteUrl}`);
        return remoteUrl;
      } else {
      ...
      this.addLog(`[TemplateConfigManager] 解析远程配置URL失败: ${err instanceof Error ? err.message : String(err)}`);
      return null;
    }
  }
  ...
  async getTemplatesIndex(): Promise<ConfigLoadResult> {
    ...
    if (this.memoryCache && timeSinceLastLoad < this.reloadInterval) {
      this.addLog(`[TemplateConfigManager] 使用内存缓存 (距离上次加载: ${Math.round(timeSinceLastLoad / 1000)}秒)`);
      this.addLog(`[TemplateConfigManager] 缓存中包含 ${Object.keys(this.memoryCache.templates).length} 个模板`);
      return { config: this.memoryCache, logs: this.getLogs() };
    }

    this.addLog('[TemplateConfigManager] 内存缓存过期或不存在，开始智能配置加载策略');
    ...
    // 1. 读取本地配置文件
    const localPath = path.join(projectRoot, 'scaffold-template', 'templates.config.json');
    this.addLog(`[TemplateConfigManager] 步骤1: 读取本地配置文件: ${localPath}`);
    
    const localConfig = await this.readJsonSafe(localPath);
    if (localConfig) {
      this.addLog('[TemplateConfigManager] ✅ 本地配置文件读取成功');
      this.addLog(`[TemplateConfigManager] 本地配置包含 ${Object.keys(localConfig.templates).length} 个模板`);
      this.addLog(`[TemplateConfigManager] 本地配置版本: ${localConfig.version}, 更新时间: ${localConfig.lastUpdated}`);
    } else {
    ...
    if (remoteConfigUrl) {
      this.addLog(`[TemplateConfigManager] 步骤2: 检查远程配置更新: ${remoteConfigUrl}`);
      try {
        remoteConfig = await this.fetchRemoteConfig(remoteConfigUrl);
        if (remoteConfig) {
          this.addLog('[TemplateConfigManager] ✅ 远程配置获取成功');
          this.addLog(`[TemplateConfigManager] 远程配置包含 ${Object.keys(remoteConfig.templates).length} 个模板`);
          this.addLog(`[TemplateConfigManager] 远程配置版本: ${remoteConfig.version}, 更新时间: ${remoteConfig.lastUpdated}`);
        }
      } catch (err) {
      ...
      this.addLog('[TemplateConfigManager] 步骤2: 本地配置中未找到远程配置信息，跳过远程配置检查');
    }
    ...
    if (remoteConfig && localConfig) {
      ...
      if (remoteTime > localTime) {
        ...
        this.addLog('[TemplateConfigManager] ✅ 本地配置为最新版本，选择本地配置');
      }
    ...
    if (selectedConfig) {
      this.addLog(`[TemplateConfigManager] ✅ 配置加载成功，来源: ${configSource}`);
    ...
    if (!selectedConfig) {
      ...
      this.addLog(`[TemplateConfigManager] 步骤4: 尝试读取缓存配置文件: ${cachedPath}`);
      
      const cachedConfig = await this.readJsonSafe(cachedPath);
      if (cachedConfig) {
        this.addLog('[TemplateConfigManager] ✅ 缓存配置文件读取成功');
        this.addLog(`[TemplateConfigManager] 缓存配置包含 ${Object.keys(cachedConfig.templates).length} 个模板`);
        this.addLog(`[TemplateConfigManager] 配置版本: ${cachedConfig.version}, 更新时间: ${cachedConfig.lastUpdated}`);
  ...
  async getTemplateEntry(name: string): Promise<UnifiedTemplateInfo | null> {
    ...
    if (template) {
      ...
      console.log(`[TemplateConfigManager] 可用模板: ${Object.keys(result.config.templates).join(', ')}`);
    }
  ...
  private async fetchRemoteConfig(url: string): Promise<any | null> {
    ...
    try {
      ...
      this.addLog(`[TemplateConfigManager] 远程请求完成 (耗时: ${fetchTime}ms, 状态: ${res.status})`);
      ...
      this.addLog(`[TemplateConfigManager] 远程配置下载成功 (大小: ${configSize} 字节)`);
      
      // 验证 JSON 格式
      try {
        const parsed = JSON.parse(json);
        this.addLog(`[TemplateConfigManager] 配置JSON解析成功，包含 ${Object.keys(parsed.templates || {}).length} 个模板`);
        return parsed;
      } catch (parseErr) {
      ...
      if (err instanceof TypeError && err.message.includes('fetch')) {
        this.addLog('[TemplateConfigManager] 网络连接失败，可能是网络问题或URL不可访问');
      } else if (err instanceof Error && err.message.includes('HTTP')) {
        this.addLog('[TemplateConfigManager] HTTP请求失败，可能是服务器问题或资源不存在');
      }
  ...
  private async cacheRemoteConfig(config: any): Promise<void> {
    try {
      this.addLog(`[TemplateConfigManager] 缓存远程配置到本地: ${this.cacheDir}`);
      ...
      await fs.writeFile(cacheFilePath, JSON.stringify(config, null, 2), 'utf-8');
  ...
  private async refreshRemoteConfig(): Promise<void> {
    this.addLog('[TemplateConfigManager] refreshRemoteConfig方法已废弃，请使用getTemplatesIndex的智能配置加载策略');
  }
  ...
  private async readJsonSafe(filePath: string): Promise<any | null> {
    try {
      ...
      this.addLog(`[TemplateConfigManager] ✅ 文件读取成功 (大小: ${content.length} 字节)`);
      return parsed;
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err);
