...
export class SmartMatcher {
  ...
  static matchTemplate(
    ...
    const { enableKeywordMatch = true, minScore = 30 } = options;
    ...
    // 第二阶段：智能积分匹配
    const smartMatch = this.findBestMatch(
      userTechStack,
      userInput,
      templates,
      minScore
  ...
  static matchMultiple(
    ...
    const { minScore = 20, maxResults = 5 } = options;
    ...
    for (const template of templates) {
      // 检查冲突和要求
      if (
        ScoreCalculator.hasConflicts(userTechStack, userInput, template) ||
  ...
  private static findBestMatch(
    ...
    let bestMatch: { template: TemplateEntry; score: MatchingScore } | null =
      null;
   ...
   * 验证模板配置
   */
  static validateTemplate(template: TemplateEntry): {
    valid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      ...
      (template.priority < 0 || template.priority > 100)
    ) {
