...
export class TemplateSync {
  ...
  ): Promise<TemplatesConfigIndex | null> {
    try {
  ...
  private shouldUpdate(
    localConfig: TemplatesConfigIndex | null,
    remoteConfig: TemplatesConfigIndex
  ): boolean {
    if (!localConfig) {
    ...
    if (localConfig.version !== remoteConfig.version) {
      this.addLog(
        `[TemplateSync] 版本不同，需要更新: ${localConfig.version} -> ${remoteConfig.version}`
      );
      return true;
    }
  ...
  private async downloadTemplate(
    ...
    try {
      ...
      this.addLog(
        `[TemplateSync] 模板 ${templateId} 下载失败: ${error.message}`
      );
      return false;
    }
  }
