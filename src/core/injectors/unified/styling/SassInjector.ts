 ...
 * 优先级: 40 (样式层)
 */
export class SassInjector extends AbstractUnifiedInjector {
  name = "sass";
  ...
  override canHandle(tools: string[]): boolean {
    return tools.some(
      (tool) => tool.toLowerCase() === "sass" || tool.toLowerCase() === "scss"
    );
  }
...
// @import './components';
`;
      this.addFile(files, "src/styles/main.scss", mainScss);
      this.addLog(logs, "生成 src/styles/main.scss");

      this.addLog(logs, "Sass 注入完成");
